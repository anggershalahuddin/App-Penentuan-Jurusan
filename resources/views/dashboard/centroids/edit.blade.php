...
@section('title', 'Centroid | Edit Centroid')
@section('container')

    {{-- ini adalah bagian Header Centroid Page - EDIT DATA --}}
    <div class="pb-2 mb-9 border-b border-secondary flex flex-row justify-between items-end">
        <h1 class="font-semibold text-slate-700 text-3xl">Edit Data Centroid
        </h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a
                            class="cursor-default ms-1 text-sm font-medium {{ Request::is('dashboard/centroids/*') ? 'text-blue-600' : '' }} hover:text-blue-600 md:ms-2">Edit
                            Centroid</a>
                    </div>
                </li>
    ...
    <form method="post" action="/dashboard/centroids/{{ $centroid->centroid_id }}">
        @method('put')
        @csrf
            ...
            <div class="grid gap-6 mb-8 grid-cols-4">
                <div>
                    ...
                    <select id="kode_centroid" name="kode_centroid"
                        ...
                        autofocus>
                        <option disabled {{ old('kode_centroid', $centroid->kode_centroid) == null ? 'selected' : '' }}
                            class="bg-gray-500 text-white">Kode Cluster</option>
                        @foreach ($kodeCentroids as $kode)
                            <option value="{{ $kode }}"
                                {{ old('kode_centroid', $centroid->kode_centroid) == $kode ? 'selected' : '' }}>
                                {{ $kode }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="nama_cluster" name="nama_cluster"
                        ...
                        placeholder="Nama Cluster" autofocus value="{{ old('nama_cluster', $centroid->nama_cluster) }}">
                    @error('nama_cluster')
                        <div class="text-red-700 mt-1 text-xs">
                            {{ $message }}
                        </div>
                ...
                <div>
                    ...
                    <select id="rumpun" name="rumpun"
                        ...
                        autofocus value="{{ old('rumpun', $centroid->rumpun) }}">
                        <option disabled {{ old('rumpun', $centroid->rumpun) == null ? 'selected' : '' }}
                            class="bg-gray-500 text-white">Kode
                            Cluster</option>
                        <option value="SAINTEK" {{ old('rumpun', $centroid->rumpun) == 'SAINTEK' ? 'selected' : '' }}>
                            SAINTEK
                        </option>
                        <option value="SOSHUM" {{ old('rumpun', $centroid->rumpun) == 'SOSHUM' ? 'selected' : '' }}>
                            SOSHUM
                        </option>
                    </select>
                    @error('rumpun')
                ...
                <div>
                    ...
                    <input list="students" name="student_id" id="student_id"
                        ...
                        placeholder="Pilih Data" value="{{ old('student_id', $centroid->student_id) }}" />
