...
@section('container')
    ...
    {{-- ini adalah bagian Header Centroid Page --}}
    <div class="pb-2 mb-9 border-b border-secondary flex flex-row justify-between items-end">
        <h1 class="font-semibold text-slate-700 text-3xl ">Data Centroid
        </h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a
                            class="cursor-default ms-1 text-sm font-medium {{ Request::is('dashboard/centroids') ? 'text-blue-600' : '' }} md:ms-2">Centroid</a>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="flex flex-row justify-between items-center mb-5">
        ...
        <div class="relative">
            ...
            <input type="text" id="table-search"
                ...
                placeholder="Cari Centroid">
        </div>
        <a href="/dashboard/centroids/create"
            class="flex flex-row items-center h-full px-4 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:bg-blue-700"
            type="button">Tambah Data
        </a>
    </div>
    ...
    <div id="table-container"
        ...
        <div>
            @include('dashboard.centroids.partials.table')
        </div>
    </div>
    {{-- Paginate Table --}}
    @include('dashboard.centroids.partials.paginate')
    ...
    @if (session()->has('success'))
        @include('dashboard.centroids.partials.alert-success')
    @endif
    ...
    <script>
        $(document).ready(function() {
            $("#table-search").on("keyup", function() {
                let query = $(this).val();
                $.ajax({
                    url: "{{ route('dashboard.centroids.index') }}",
                    type: "GET",
                    data: {
                        search: query,
                    },
